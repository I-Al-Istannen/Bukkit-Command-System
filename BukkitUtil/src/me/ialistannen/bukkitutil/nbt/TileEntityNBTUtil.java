...
public class TileEntityNBTUtil {
	...
	static {
		...
		if (!getTileEntityOpt.isPresent()) {
			PluginMain.getInstance().getLogger().warning("getTileEntity not found. Version: "
					+ Bukkit.getBukkitVersion() + " " + Bukkit.getVersion());
			error = true;
		}
		else {
			getTileEntity = getTileEntityOpt.get();
			initializeMethods();
		}
	}
	...
	private static Object toTileEntity(BlockState blockState) {
		ensureNoError();
		return ReflectionUtil.invokeMethod(getTileEntity, blockState);
	}
	...
	private static void initializeMethods() {
		...
		Object tileEntity = ReflectionUtil.invokeMethod(getTileEntity, chestState);

		if (ReflectionUtil.getMajorVersion() > 2 || ReflectionUtil.getMinorVersion() > 9) {
			initializeMethodsAfter1_9(ReflectionUtil.getNMSClass("TileEntity"), chestState, tileEntity);
		}
		else {
			initializeMethodsBefore1_9(ReflectionUtil.getNMSClass("TileEntity"), chestState, tileEntity);
		}

		if (loadFromNBT == null || saveToNBT == null) {
			PluginMain.getInstance().getLogger().warning("Null: "
	...
	private static void initializeMethodsAfter1_9(Class<?> tileEntityClass, BlockState blockState,
		...
		for (Method method : tileEntityClass.getMethods()) {
			if (method.getReturnType().equals(ReflectionUtil.getNMSClass("NBTTagCompound"))
				...
				if (saveToNBT != null) {
					PluginMain.getInstance().getLogger().warning("Found more than one save method. Version: "
							+ Bukkit.getBukkitVersion() + " " + Bukkit.getVersion());
					error = true;
					return;
				}
		...
		for (Method method : tileEntityClass.getMethods()) {
			if (method.getReturnType().equals(Void.TYPE)
				...
				if (compound.isEmpty()) {
					// load method
					if (loadFromNBT != null) {
						PluginMain.getInstance().getLogger().warning("Found more than one load method. Version: "
								+ Bukkit.getBukkitVersion() + " " + Bukkit.getVersion());
						error = true;
						return;
					}
